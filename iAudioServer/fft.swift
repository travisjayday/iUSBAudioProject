...
class FFT {
    ...
    static let overlapSize = 16
    var lastSamples = Array(repeating: 0.0, count: overlapSize)
    ...
    func calculate(_ _values: [Double], fps: Double) -> [Double] {
        ...
        values = lastSamples + values
        
        lastSamples = _values
        lastSamples.removeFirst(lastSamples.count - FFT.overlapSize)
        ...
        let last = mag[mag.count - 1]
        mag.removeFirst(2)
        mag.append(contentsOf: Array(repeating: last, count: 2))
        ...
        var extendedWindow = lastSamples + mag
        
        /* Pitch incrase
            mag.removeLast(5)
            mag = Array(repeating: 0, count: 5) + mag
            print(mag.count)
        */
        ...
        result.removeFirst(FFT.overlapSize)
        return result
    }
